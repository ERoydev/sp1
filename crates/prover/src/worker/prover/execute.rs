...
pub async fn execute_with_options(
    ...
    //   // Diagnostic: always prints to stderr (no RUST_LOG / tracing needed)
    // eprintln!(
    //     "[SP1 ExecutionReport] profiling_cfg={} cycle_tracker_len={} keys={:?}",
    //     cfg!(feature = "profiling"),
    //     final_report.cycle_tracker.len(),
    //     final_report.cycle_tracker.keys().collect::<Vec<_>>()
    // );
